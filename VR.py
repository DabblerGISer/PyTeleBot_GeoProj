...
def beep_bot(update, context):
	CID = update.message.chat_id
	if update.message.text.lower() == "beep":
		context.bot.sendMessage(CID, "`Boop`", parse_mode="Markdown")
	else:
		context.bot.sendMessage(CID, "`I only work through inline mode, write @VorpalRobot followed by a space to try it out.`", parse_mode="Markdown")

updater = Updater(token='2068690659:AAEfQ150bLudfpMe5QbKGJS8liTip3gU4U0')
dispatcher = updater.dispatcher
dispatcher.add_handler(MessageHandler(Filters.text, callback=beep_bot))
